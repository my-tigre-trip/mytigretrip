...
if(!function_exists('mkdf_tours_version_class')) {
	...
	function mkdf_tours_version_class($classes) {
		$classes[] = 'mkdf-tours-'.MIKADO_TOURS_VERSION;
	...
	function mkdf_tours_get_tour_attributes() {
		...
		if(mkdf_tours_is_wpml_installed()) {
			...
			$sql = "SELECT p.*
					...
					WHERE p.post_type = 'tour-attributes'
		...
		foreach($sections_array as $section) {
			...
			switch($section_key) {
				...
				case 'show_custom_section_2' :
					$custom_section2_title = (get_post_meta(get_the_ID(), 'mkdf_tours_custom_section2_title', true) != '') ? get_post_meta(get_the_ID(), 'mkdf_tours_custom_section2_title', true) : esc_html__('Custom Section 2', 'mikado-tours');
					$return_array[$section_key]['icon']  = 'icon_book';
...
if(!function_exists('mkdf_tours_is_wpml_installed')) {
	/**
	 ...
	 * @version 0.1
	 */
	function mkdf_tours_is_wpml_installed() {
...
if(!function_exists('mkdf_tours_get_checkout_pages')) {
	...
	function mkdf_tours_get_checkout_pages($first_empty = false) {
		...
		if($first_empty) {
			$search_pages[''] = esc_html__('No Selected', 'mikado-tours');
		}
...
if(!function_exists('mkdf_tours_get_search_page_url')) {
	...
	function mkdf_tours_get_search_page_url() {
	    ...
	    $option = gotravel_mikado_options()->getOptionValue('tours_search_main_page');
...
if(!function_exists('mkdf_tours_paypal_enabled')) {
	...
	function mkdf_tours_paypal_enabled() {
	    ...
	    if(mkdf_tours_theme_installed()) {
		    $option = gotravel_mikado_options()->getOptionValue('tours_enable_paypal');
	    }
