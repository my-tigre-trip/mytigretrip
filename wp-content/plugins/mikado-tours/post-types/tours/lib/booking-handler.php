...
class BookingHandler {
	...
	public function handleBooking() {
		...
		if(!$returnObject->status) {
			...
			$returnObject->messages[] = esc_html__('Booking successful', 'mikado-tours');

			if(mkdf_tours_theme_installed()) {
				$redirectOptionValue = gotravel_mikado_options()->getOptionValue('tours_checkout_page');
	...
	public function loadAssets() {
		if(is_singular('tour-item')) {
			...
			wp_enqueue_script('mkdf-tour-booking', plugins_url(MIKADO_TOURS_REL_PATH.'/assets/js/tour-booking.js'), $deps, false, true);
		}
	}
	...
	private function sendAdminConfirmationEmail($id, $bookingEmail, $bookingHash) {
		...
		if(gotravel_mikado_options()->getOptionValue('enable_admin_booking_email') == 'yes'){
			//Get email address
			if(gotravel_mikado_options() -> getOptionValue('admin_email') != '') {
				$mail_to = gotravel_mikado_options() -> getOptionValue('admin_email');
			} else {
			...
			$message = esc_html__('You have received the reservation for','mikado-tours') .  ' '. $tour_title . ' .';
