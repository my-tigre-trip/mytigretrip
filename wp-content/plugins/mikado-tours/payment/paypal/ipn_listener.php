...
$params = array(
	...
	'compress'    => false,
	'decompress'  => false,
	'user-agent'  => 'MikadofTours/' . MIKADO_TOURS_VERSION
);
...
if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 && strstr( $response['body'], 'VERIFIED' ) ) {
	...
	if ( mkdf_tours_theme_installed() && gotravel_mikado_options()->getOptionValue( 'paypal_facilitator_id' ) == $receiver_email ) {
		...
		if ( $payment_status == 'completed' && $post_data['test_ipn'] == 1 ) {
			...
			mkdf_tours_paypal_payment_update_booking( $booking_data, $payment_data );
		}
	}
}