 ...
 */
if(!class_exists('GoTravelMikadoStickyNavigationWalker')) {
	class GoTravelMikadoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($this->background_image !== '') {
				$menu_image_background  = "background-image:url(".$this->background_image.");";
				$background_image_class = ' mkdf-dropdown-with-background-image';
			}

			$indent = str_repeat("\t", $depth);
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.gotravel_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
