...
if($row_type == 'row' && $css_animation != '') {
	...
	if($transition_delay !== '') {
		...
		$inner_start .= '<div '.gotravel_mikado_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	} else {
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($video == 'show_video') {
	...
	if($video_image) {
		$video_output .= '<div class="mkdf-mobile-video-image" '.gotravel_mikado_get_inline_attr($video_mobile_style, 'style').')"></div>';
	}
	$video_output .= '<div '.gotravel_mikado_get_class_attribute($video_overlay_class).gotravel_mikado_get_inline_attr($video_overlay_style, 'style').'></div>';
	$video_output .= '<div class="mkdf-video-wrap">';
	$video_output .= '<video class="mkdf-video" width="1920" height="800" '.gotravel_mikado_get_inline_attr($video_attrs, 'poster').' controls="controls" preload="auto" loop autoplay muted>';
	if(!empty($video_webm)) {
	...
	if($v_image) {
		$video_output .= '<img '.gotravel_mikado_get_inline_attr($v_image, 'src').' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('Video thumb', 'gotravel').'" />';
	}
	$video_output .= '</object>';
	$video_output .= '</video>';
	$video_output .= '</div>';
