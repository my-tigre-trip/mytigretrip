...
class GoTravelMikadoLike {
	private static $instance;

	private function __construct() {
		add_action('wp_enqueue_scripts', array( $this, 'enqueue_scripts'));
		add_action('wp_ajax_gotravel_mikado_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_gotravel_mikado_like', array( $this, 'ajax'));
	}
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'gotravel');
		if( isset($_COOKIE['mkdf-like_'. $post->ID]) ){
			$class = 'mkdf-like liked';
			$title = esc_html__('You already like this!', 'gotravel');
		}
...
if (!function_exists('gotravel_mikado_create_like')) {
	function gotravel_mikado_create_like() {
		GoTravelMikadoLike::get_instance();
	}
	
	add_action('after_setup_theme', 'gotravel_mikado_create_like');
}
