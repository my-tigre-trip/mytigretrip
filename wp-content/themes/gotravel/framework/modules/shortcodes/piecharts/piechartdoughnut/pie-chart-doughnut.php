<?php
namespace GoTravel\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Value ', 'gotravel') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
				'heading' => esc_html__('Chart Color ', 'gotravel') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Legend ', 'gotravel') . $i,
		...
		vc_map( array(
			'name' => esc_html__('Mikado Pie Chart 3 (Doughnut)', 'gotravel'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Height', 'gotravel'),
						'param_name' => 'height',
					),
					array(
						'type'        => 'dropdown',
	...
	public function render($atts, $content = null) {
		...
		$html = gotravel_mikado_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
	...
	private function getHolderClasses($params) {
		$classes = array('mkdf-pie-chart-doughnut-holder');
