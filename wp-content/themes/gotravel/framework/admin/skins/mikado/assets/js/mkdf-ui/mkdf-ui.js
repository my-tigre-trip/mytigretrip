(function ($) {
    ...
    function mkdChangedInput() {
        ...
        $('.mkdf-tabs-content form.mkd_ajax_form:not(.mkdf-import-page-holder) #anchornav input').click(function () {
            ...
            setTimeout(function () {
                $('.mkdf-changes-saved').removeClass('yes');
            }, 3000);
        });
    }
            ...
            frame: function () {
                ...
                this._frame.on('update', function () {
                    ...
                    jQuery.ajax({
                        type: "post",
                        url: ajaxurl,
                        data: "action=gotravel_mikado_gallery_upload_get_images&ids=" + ids,
    ...
    function mkdfInitAjaxForm() {
        ...
        $(document).delegate(".mkd_ajax_form", "submit", function (a) {
            var b = $(this);
            var c = {
                action: "gotravel_mikado_save_options"
            };
            jQuery.ajax({
                url: ajaxurl,
                cache: !1,
    ...
    function mkdfRepeater() {
        var addNew = $('.mkdf-repeater-add .mkdf-clone'); // add new button
        var removeBtn = $('.mkdf-clone-remove');
        if (addNew.length) {
            $('.mkdf-repeater-fields-holder').each(function () {
                var thisHolderRows = $(this).find('.mkdf-repeater-fields-row');
            ...
            addNew.on('click', function (e) {
                ...
                var fieldsHolder = thisAddNew.parent().siblings('.mkdf-repeater-fields-holder'); // container of all rows - parent to append new row
                var rows = fieldsHolder.find('.mkdf-repeater-fields-row');
                ...
                if (append) {
                    ...
                    firstChild.find('.mkdf-repeater-field').each(function () {
                            ...
                            thisField.find(':input, textarea').each(function () {
                                ...
                                if (thisInput.hasClass('my-color-field')) { // if input type is color picker
                                ...
                                else if (thisInput.hasClass('mkdf-wp-editor-area')) {
                                    textareahtml = true;
                                    ...
                                    if (typeof id !== 'undefined') {
                                        ...
                                        setTimeout(function () {
                                            var newTextArea = textareaHolder.find('#' + textareaID);
                                            newTextArea.wp_editor();
                                        }, 300);
                                    }
                                }
        ...
        function removeRow(removeBtn) {
            removeBtn.on('click', function (e) {
                ...
                var parentRow = thisRemoveBtn.parents('.mkdf-repeater-fields-row');
                if (parentRow.is(':first-child') && thisRemoveBtn.parents('.mkdf-repeater-fields-holder').find('.mkdf-repeater-fields-row').length == 1) {
    ...
    function mkdfInitWPEditor() {
        var editors = $('.mkdf-wp-editor-area');
        if (editors.length) {
