...
/*
  Class: GoTravelMikadoIconCollections
...
class GoTravelMikadoIconCollections {
	...
	private function initIconCollections() {
		$this->addIconCollection('font_awesome', new GoTravelMikadoIconsFontAwesome("Font Awesome", "fa_icon"));
		$this->addIconCollection('font_elegant', new GoTravelMikadoIconsFontElegant("Font Elegant", "fe_icon"));
		$this->addIconCollection('ion_icons', new GoTravelMikadoIonIcons("Ion Icons", "ion_icon"));
		$this->addIconCollection('linea_icons', new GoTravelMikadoLineaIcons('Linea Icons', 'linea_icon'));
		$this->addIconCollection('linear_icons', new GoTravelMikadoLinearIcons('Linear Icons', 'linear_icon'));
	}
	...
	public function getSocialVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false, $exclude = '') {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__('Icon Pack', 'gotravel'),
		...
		if(is_array($iconCollections) && count($iconCollections)) {
			foreach($iconCollections as $key => $collection) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__('Icon', 'gotravel'),
					'param_name'  => $iconCollectionPrefix.$collection->param,
	...
	public function getWidgetIconParams() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__('Icon Pack', 'gotravel'),
			'name'    => 'icon_pack',
			'options' => array(
				'font_awesome'      => esc_html__('Font Awesome', 'gotravel'),
				'font_elegant'      => esc_html__('Font Elegant', 'gotravel'),
				'ion_icons'         => esc_html__('Ion Icons', 'gotravel')
			)
		);
	...
	public function getIconCollectionIcons(iGoTravelMikadoIconCollection $collection) {
...
if (!function_exists('gotravel_mikado_activate_theme_icons')) {
	function gotravel_mikado_activate_theme_icons() {
		global $gotravel_mikado_IconCollections;
		$gotravel_mikado_IconCollections = GoTravelMikadoIconCollections::get_instance();
	}
	
	add_action('after_setup_theme', 'gotravel_mikado_activate_theme_icons');
}