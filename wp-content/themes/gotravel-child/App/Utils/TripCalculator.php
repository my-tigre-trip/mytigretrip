...
class TripCalculator {
    ...
    */
    ...
    public function boatPrice($boat, $adults, $children = 0) {
      $prices = $this->P->findBoatPrices($boat); 
      
      $people = $adults + $children;
      $basePrice = $this->boatBasePrice($boat, $prices);

      $priceAdultsDiscount = $this->boatAdultsPrice($boat, $adults, $prices);

      $priceChildrenDiscount = 0;
      if (intval($children) > 0) {
        $priceChildrenDiscount = $this->boatChildrenPrice($boat, $children, $prices);
      }
      ...
      $groupDiscount = $priceBeforeDiscount-$price;
       ...
       return [
           ...
           'children' => $children,
           'price' => $price,
           'priceChildren' => $basePrice*$children,
           'discountChildren' => $priceBeforeDiscount - $priceChildrenDiscount,
           'priceAdults' => $basePrice*$adults,
  ...
  */
  public function calculatePriceFromQuery($req, $boat, $myTrip = null) {
    ...
    $car = isset($req['car']) ? $req['car'] : false;
    $islandExpenses = isset($req['island_expenses']) ? $req['island_expenses'] : null;

    $boatPrice = $this->boatPrice($boat, $adults, $children);
    // if has stops
